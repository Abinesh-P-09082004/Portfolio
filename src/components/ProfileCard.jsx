...
// --- UTILITY FUNCTIONS (from ProfileCard) ---
const DEFAULT_INNER_GRADIENT = 'linear-gradient(145deg, rgba(15, 15, 15, 0.95) 0%, rgba(0, 0, 0, 0.9) 100%)';
...
const ProfileCardComponent = ({
  avatarUrl = '/ABINESH.png', iconUrl = 'none', grainUrl = 'none', innerGradient,
  behindGlowEnabled = true, behindGlowColor, behindGlowSize, className = '',
  enableTilt = true, enableMobileTilt = false, mobileTiltSensitivity = 5,
  name = 'ABINESH P', title = 'Frontend Developer',
  handle = 'ABINESH_P',
}) => {
  ...
  const tiltEngine = useMemo(() => {
    ...
    let rafId = null; let running = false; let lastTs = 0;
    let currentX = 0; let currentY = 0; let targetX = 0; let targetY = 0;
    const DEFAULT_TAU = 0.14; const INITIAL_TAU = 0.6; let initialUntil = 0;

    const setVarsFromXY = (x, y) => {
      const shell = shellRef.current; const wrap = wrapRef.current;
      if (!shell || !wrap) return;

      const width = shell.clientWidth || 1; const height = shell.clientHeight || 1;
      const percentX = clamp((100 / width) * x); const percentY = clamp((100 / height) * y);
      const centerX = percentX - 50; const centerY = percentY - 50;
  ...
  const handlePointerEnter = useCallback(event => {
    const shell = shellRef.current;
    if (!shell || !tiltEngine) return;
    shell.classList.add('active'); shell.classList.add('entering');
  ...
  const handleDeviceOrientation = useCallback(event => {
    ...
    const centerX = shell.clientWidth / 2; const centerY = shell.clientHeight / 2;
    const x = clamp(centerX + gamma * mobileTiltSensitivity, 0, shell.clientWidth);
    const y = clamp(
      centerY + (beta - ANIMATION_CONFIG.DEVICE_BETA_OFFSET) * mobileTiltSensitivity,
      0, shell.clientHeight
    );
    tiltEngine.setTarget(x, y);
  ...
  const cardStyle = useMemo(() => ({
    ...
    '--behind-glow-color': behindGlowColor ?? 'rgba(137, 176, 174, 0.67)',
      ...
      <div ref={shellRef} className="pc-card-shell">
        <section className="pc-card">
          <div className="pc-inside">
            ...
            {/* NAME, HANDLE, and TITLE Content Block */}
            <div className="pc-content pc-details-content">
              <div className="pc-details">
                <h3>{name}</h3>
                {/* Re-introducing the handle here */}
                <div className="pc-handle-top">@{handle}</div>
                <p>{title}</p>
              </div>
            </div>
